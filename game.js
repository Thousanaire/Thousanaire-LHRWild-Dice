...
function startIntroOverlay() {
  ...
  // If overlay missing, skip intro entirely
  if (!overlay) return;

  // Ensure overlay is visible
  overlay.style.display = "flex";

  // Add subtle float if avatar exists
  if (avatar) {
  ...
  // If audio exists, set up playback + mobile unlock
  if (voice) {
    ...
    // Attempt autoplay (desktop)
    voice.play().catch(() => {
      // If blocked, overlay click will start it
    });
  }
}
...
function handleThreeWildSteals(playerIndex) {
  ...
  function renderStealPanel() {
    ...
    if (stealsRemaining === 0) {
      ...
      wildContent.appendChild(finish
